
/**
 * Write a description of class PhotoPrintsMain here.
 *
 * @author (your name)
 * @version (a version number or a date)
 */
import java.util.Scanner;
import java.io.File;
import java.util.StringTokenizer;

public class PhotoPrintsMain3
{
    public static void main (String[] args)
    {
        ArrayList printList = new ArrayList(50);
        
        File inputFile = new File("PhotoData.txt");
        
        try {
             Scanner inFile = new Scanner(inputFile); 
             while (inFile.hasNextLine()) {
                String s = inFile.nextLine(); 
                String delimiter = ";"; 
                StringTokenizer st = new StringTokenizer(s, delimiter);
                 
                String method = st.nextToken();
                char paperType = st.nextToken().charAt(0);
                int photoSize = Integer.parseInt(st.nextToken());
                int quantity = Integer.parseInt(st.nextToken());
                             
                PhotoPrints c = new PhotoPrints(method,paperType, photoSize, quantity);
                printList.insertEnd(c);
              }
            }catch (Exception e)             
            {           
                System.out.println(e);       
            }
        
        /* Scanner s = new Scanner(System.in);
        for (int i = 0; i < 1; i++)
        {
            System.out.println("Method:");
            String m = s.nextLine();
            System.out.println("Type of paper:");
            char pt = s.nextLine().charAt(0);
            System.out.println("Size of photo:");
            int ps = Integer.parseInt(s.nextLine());
            System.out.println("Quantity:");
            int q = Integer.parseInt(s.nextLine());
            
            PhotoPrints print = new PhotoPrints(m, pt, ps, q);
            printList.add(print);
            
        }
        */
       
        System.out.println("\nAll PhotoPrint");
        System.out.println("\n==========");
        for (int i = 0; i < printList.getNum(); i++)
        {
            System.out.println(printList.getData(i).toString());
        }
        
        
        //ii
        int count = 0;
        for (int i = 0; i < printList.getNum(); i++)
        {
            PhotoPrints pp = (PhotoPrints) printList.getData(i);
            if (pp.getPhotoSize() == 11)
            {
                count += pp.getQuantity();
            }
        }
        
        System.out.println("The number of photo prints with the size of 11R is: " + count);
        
        //iii
        ArrayList classicList = new ArrayList(50);
        ArrayList digitalList = new ArrayList(50);
        
        for (int i = 0; i < printList.getNum(); i++)
        {
            PhotoPrints pp = (PhotoPrints) printList.getData(i);
            if (pp.getMethod().equalsIgnoreCase("classic"))
            {
                classicList.insertEnd(pp);
                printList.removeAt(i);
            }
            else
            {
                digitalList.insertEnd(pp);
                printList.removeAt(i);
            }
            i--;
        }
        
        System.out.println(classicList.print());
        System.out.println(digitalList.print());
        
        int totalQuantity = 0;
        for (int i = 0; i < classicList.getNum(); i++)
        {
            PhotoPrints pp = (PhotoPrints) classicList.getData(i);
            if (pp.getPaperType() == 'G')
            {
                totalQuantity += pp.getQuantity();
            }
        }
        
        System.out.println("The total quantity of classic prints using gloss paper type: " + totalQuantity);        
    }
}

